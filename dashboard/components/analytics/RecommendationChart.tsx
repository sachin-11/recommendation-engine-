...
import type { TokenUsage, UsageResponse } from "@/types";

// Chart colours come from CSS variables, so they follow the light/dark theme.
export const CHART_COLORS = [1, 2, 3, 4, 5, 6].map(
  (n) => `hsl(var(--chart-${n}))`,
);
...
/** Daily tokens, stacked: uploads (item embedding) and queries. */
export function TokenChart({ daily }: { daily: TokenUsage["daily"] }) {
  const data = daily.map((d) => ({ ...d, label: shortDate(d.date) }));
  return (
    <ResponsiveContainer width="100%" height={260}>
...
export function QueryTypeChart({
  ...
  byType: UsageResponse["by_query_type"];
}) {
  const data = Object.entries(byType).map(([type, count]) => ({
    type: QUERY_LABELS[type] ?? type,
    count,
  }));
  return (
    <ResponsiveContainer width="100%" height={260}>
